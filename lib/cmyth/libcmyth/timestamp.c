...
cmyth_timestamp_utc_from_unixtime(time_t l)
{
	...
	cmyth_timestamp_t ts = cmyth_timestamp_from_tm(&tm_datetime);
...
cmyth_timestamp_to_utc(cmyth_timestamp_t ts)
{
	...
	time_t l = cmyth_timestamp_to_unixtime(ts);
	struct tm tm_datetime;
	gmtime_r(&l, &tm_datetime);
	cmyth_timestamp_t ret = cmyth_timestamp_from_tm(&tm_datetime);
	ret->timestamp_isutc = 1;
	return ret;
}
