 ...
 */
int8_t
cmyth_mysql_get_recording_seek_offset(cmyth_database_t db, cmyth_proginfo_t prog, int64_t mark, int64_t *psoffset, int64_t *nsoffset)
{
	MYSQL_RES *res = NULL;
	MYSQL_ROW row;
	/*
	 ...
	 * const DESC, const DESC, const DESC
	 */
	const char *query_str1 = "SELECT mark,offset FROM recordedseek WHERE chanid = ? AND starttime = ? AND type = 9 AND mark >= ? ORDER BY chanid ASC, starttime ASC, type ASC, mark ASC LIMIT 1;";
	const char *query_str2 = "SELECT mark,offset FROM recordedseek WHERE chanid = ? AND starttime = ? AND type = 9 AND mark <= ? ORDER BY chanid DESC, starttime DESC, type DESC, mark DESC LIMIT 1;";
	int64_t next_mark = 0;
