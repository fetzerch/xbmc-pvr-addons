...
cmyth_mysql_get_chanlist(cmyth_database_t db, cmyth_chanlist_t *chanlist)
{
	...
	const char *query_str = "SELECT c.chanid, c.channum, c.name, c.icon, c.visible, c.sourceid, c.mplexid, c.callsign, "
			"COALESCE(scan.is_audio_service, 0) AS is_audio_service "
			"FROM channel AS c LEFT JOIN ("
			"SELECT is_audio_service, service_id, MAX(scandate) "
			"FROM channelscan JOIN channelscan_channel ON channelscan.scanid = channelscan_channel.scanid "
			"GROUP BY (service_id) "
			") scan ON c.serviceid = scan.service_id;";
	int rows = 0;
	cmyth_mysql_query_t * query;
