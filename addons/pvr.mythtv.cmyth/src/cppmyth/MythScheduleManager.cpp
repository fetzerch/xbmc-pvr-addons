...
void MythScheduleManager::Setup()
{
  ...
  // On new DB connection the schema version could change. So changes to the corresponding helper
...
MythScheduleManager::MSM_ERROR MythScheduleManager::UpdateRecording(unsigned int index, MythRecordingRule &newrule)
{
  ...
  {
    METHOD_UNKNOWN = 0,
    METHOD_DISCREET_UPDATE = 1,
    METHOD_CREATE_OVERRIDE = 2,
    METHOD_FULL_UPDATE = 3
  };
...
ScheduleList MythScheduleManager::FindUpComingByRuleId(unsigned int recordID) const
{
  ...
  if (range.first != m_recordingIndexByRuleId.end())
    for (RecordingIndexByRuleId::const_iterator it = range.first; it != range.second; ++it)
      found.push_back(std::make_pair(it->second, m_recordings.at(it->second).get()));
  return found;
}
...
MythRecordingRule MythScheduleManager::NewChannelRecord(CStdString searchTitle)
{
  return m_versionHelper->NewChannelRecord(searchTitle);
}

MythRecordingRule MythScheduleManager::NewOneRecord(CStdString searchTitle)
{
...
MythRecordingRule MythScheduleHelperNoHelper::NewChannelRecord(CStdString searchTitle)
{
  (void)searchTitle;
  return MythRecordingRule();
}

MythRecordingRule MythScheduleHelperNoHelper::NewOneRecord(CStdString searchTitle)
{
  (void)searchTitle;
  return MythRecordingRule();
}
...
MythRecordingRule MythScheduleHelper1226::NewChannelRecord(CStdString searchTitle)
{
...
MythRecordingRule MythScheduleHelper1226::NewOneRecord(CStdString searchTitle)
{
  // Backend use the description to find program by keywords or title
  MythEPGInfo epgInfo;
  MythRecordingRule rule = this->NewFromTemplate(epgInfo);
