...
MythConnection::MythConnection(const CStdString &server, unsigned short port)
  ...
  , m_pEventHandler(NULL)
{
  cmyth_conn_t connection = cmyth_conn_connect_ctrl(const_cast<char*>(server.c_str()), port, RCVBUF_SIZE, TCPBUF_SIZE);
  *m_conn_t = connection;
}
...
MythFile MythConnection::ConnectFile(MythProgramInfo &recording)
{
  ...
  CMYTH_CONN_CALL_REF(file, true, cmyth_conn_connect_file(*recording.m_proginfo_t, *m_conn_t, RCVBUF_SIZE, TCPBUF_SIZE));
...
{
  cmyth_file_t file = NULL;
  CMYTH_CONN_CALL_REF(file, file == NULL, cmyth_conn_connect_path(const_cast<char*>(filename.c_str()), *m_conn_t, RCVBUF_SIZE, TCPBUF_SIZE, const_cast<char*>(storageGroup.c_str())));
