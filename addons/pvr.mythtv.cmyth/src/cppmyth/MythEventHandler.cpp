...
{
  *m_conn_t = cmyth_conn_connect_event(const_cast<char*>(m_server.c_str()), port, RCV_BUF_SIZE, TCP_RCV_BUF_SIZE);
}
...
void MythEventHandler::MythEventHandlerPrivate::RetryConnect()
{
  ...
  while (!IsStopped())
  {
    ...
    ref_release(*m_conn_t);
    *m_conn_t = NULL;
    *m_conn_t = cmyth_conn_connect_event(const_cast<char*>(m_server.c_str()), m_port, RCV_BUF_SIZE, TCP_RCV_BUF_SIZE);
