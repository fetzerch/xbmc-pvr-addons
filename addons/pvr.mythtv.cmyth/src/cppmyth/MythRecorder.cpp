  ...
  *m_liveChainUpdated = 0;
  ...
  if (recorder && pErr == NULL) {
    *m_recorder_t = recorder;
    ...
    if (*m_liveChainUpdated == 0)
    {
      ...
      // Notify 'No response from MythTV backend'
      XBMC->QueueNotification(QUEUE_ERROR, XBMC->GetLocalizedString(30304));
    }
    else
      ret = true;
  }
  else
  {
    ...
    *m_recorder_t = NULL;
    ...
    // Notify 'Recorder unavailable'
    XBMC->QueueNotification(QUEUE_ERROR, XBMC->GetLocalizedString(30305));
  }
