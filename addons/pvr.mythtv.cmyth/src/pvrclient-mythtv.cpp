...
void PVRClientMythTV::EventUpdateRecordings()
{
  ...
  while (true)
  {
    ...
    switch (event.Type())
    {
      ...
      {
        MythProgramInfo prog = m_con.GetRecordedProgramFromTs(event.ChannelID(), event.RecStartTs());
        if (!prog.IsNull())
        {
          ...
          if (it == m_recordings.end())
          {
            ...
            // Add an item with the new program
            m_recordings.insert(std::pair<CStdString, MythProgramInfo>(prog.StrUID().c_str(), prog));
          }
        }
        else
          XBMC->Log(LOG_ERROR, "%s - Add recording failed for %i %ld", __FUNCTION__, event.ChannelID(), event.RecStartTs());
        break;
      }
      case MythEventHandler::CHANGE_UPDATE:
      {
        ...
        if (it != m_recordings.end())
        {
          ...
          // Copy old pinned values
          prog.SetFramerate(it->second.Framterate());
          // Fill artworks
          m_db.FillRecordingArtwork(prog);
          // Update item with the new program
          it->second = prog;
        }
        break;
      }
      ...
      {
        // MythTV send two DELETE events. First request deletion, second confirms deletion.
...
PVR_ERROR PVRClientMythTV::DeleteRecording(const PVR_RECORDING &recording)
{
  ...
  CLockObject lock(m_recordings_lock);
