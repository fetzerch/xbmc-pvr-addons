...
{
  bool retval;
  ChannelIdMap::iterator channelByIdIt;
  ...
  //Find channel to switch.
  channelByIdIt = m_channelsById.find(channelinfo.iUniqueId);
  if (channelByIdIt == m_channelsById.end())
  {
    XBMC->Log(LOG_ERROR,"%s - Channel not found", __FUNCTION__);
    return false;
  }
  //Recorder is recording and channel is tunable then use SET_CHANNEL method.
