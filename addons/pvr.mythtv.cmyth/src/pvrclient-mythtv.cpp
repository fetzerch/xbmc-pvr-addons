...
PVR_ERROR PVRClientMythTV::GetTimers(ADDON_HANDLE handle)
{
  ...
  // Clear old status before make the new.
  ...
  for (ScheduleList::iterator it = upcomingRecordings.begin(); it != upcomingRecordings.end(); ++it)
  {
    ...
    // Is there a rule for recording ? Then fill rule info
    ...
    if (node)
    {
      ...
      // Is rule is repeating ? Then set bIsRepeating and iweekdays
    ...
    case RS_UNKNOWN:
      // Not recording !
      rulemarker.append("(").append(XBMC->GetLocalizedString(30312)).append(")");
    ...
    // Title
    // It must contains at the begining the original title. cf PVRtoMythRecordingRule()
    // String will be compare with EPG title to check is custom or not.
    ...
    m_PVRtimerMemorandum.insert(std::make_pair(tag.iClientIndex, tag));
  }
  ...
  else
  {
    ...
    if (!timer.bIsRepeating)
    {
      if (m_db.FindProgram(st, timer.iClientChannelUid, "%", epgInfo) && title.compare(0, epgInfo.Title().length(), epgInfo.Title()) == 0)
        ...
        epgInfo = MythEPGInfo();
      // We can search for this title...
      //if (!epgFound)
      //  rule = m_scheduleManager->NewChannelRecord(timer.strTitle);
      //else
      // Or make a manual single record
      rule = m_scheduleManager->NewSingleRecord(epgInfo);
    }
  }
...
PVR_ERROR PVRClientMythTV::UpdateTimer(const PVR_TIMER &timer)
{
  ...
  std::map<unsigned int, PVR_TIMER>::const_iterator old = m_PVRtimerMemorandum.find(timer.iClientIndex);
...
PVR_ERROR PVRClientMythTV::CallMenuHook(const PVR_MENUHOOK &menuhook, const PVR_MENUHOOK_DATA &item)
{
  ...
  if (menuhook.iHookId == MENUHOOK_KEEP_LIVETV_RECORDING && item.cat == PVR_MENUHOOK_RECORDING)
  {
    ...
    else
    {
      ...
      {
        CStdString info = CStdString(XBMC->GetLocalizedString(menuhook.iLocalizedStringId));
