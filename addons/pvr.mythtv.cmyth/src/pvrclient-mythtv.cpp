...
PVR_ERROR PVRClientMythTV::DeleteTimer(const PVR_TIMER &timer, bool bForceDelete)
{
  ...
  // Delete related Override and Don't Record rule
  std::vector<RecordingRule*> modifiers = rule.GetModifiers();
  for (std::vector <RecordingRule*>::iterator it = modifiers.begin(); it != modifiers.end(); ++it)
  {
    // Stop recording scheduled by the modifier before delete (issue #45)
  ...
  // Delete parent rule
  // Stop recording scheduled by the rule (issue #45)
